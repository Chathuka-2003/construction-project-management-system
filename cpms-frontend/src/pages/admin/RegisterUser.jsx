...
// ✅ MUST match backend Role enum values
// Your backend uses OTHER_STAFF (not STAFF)
const ROLES = ["ADMIN", "MANAGER", "ENGINEER", "OTHER_STAFF", "WORKER", "CUSTOMER"];
...
export default function RegisterUser() {
  ...
  async function onSubmit(e) {
    ...
    try {
      ...
      const createdUser = await adminRegisterUser(payload);

      toast.success(
        createdUser?.email ? `User created: ${createdUser.email}` : "User registered successfully!"
      );
...
function extractBackendError(err) {
  ...
  if (status === 403) return "Access denied. You don't have permission to register users.";
